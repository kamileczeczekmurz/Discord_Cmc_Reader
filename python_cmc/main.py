...
async def on_message(message):
    if message.author == bot.user:
        return
    if 'murzyni sa super' in message.content.lower():
        await asyncio.sleep(1)
        await message.delete()
        await message.channel.send(f'{message.author.mention} ziomus nie pojebalo cie?')
...
async def remove(ctx, member: discord.Member = None):
    ...
    if role not in member.roles:
        await ctx.send(f"{member.mention} doesn't have the role 'test_role'")
        return
...
async def bum(ctx):
    await ctx.send(f"Witam Krula {ctx.author.mention}")
